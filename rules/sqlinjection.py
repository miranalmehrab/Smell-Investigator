...
def detect(token, project_name, src_file):
    try:
        ...
        if tokenType == "variable" and token.__contains__('valueSrc') and token.__contains__('args'):
            ...
            if valueSrc in queryMethods and token['isInput'] is True:
        ...
        elif tokenType == "function_call" and name in queryMethods and token['hasInputs'] is True:
            action_upon_detection(project_name, src_file, lineno, 'constructing sql statement upon user input', 'constructing sql statement upon user input', token)
        
        elif tokenType == "function_def" and token.__contains__('return') and token.__contains__('returnHasInputs'): 
            if token['return'] in queryMethods and token['returnHasInputs'] is True:
                action_upon_detection(project_name, src_file, lineno, 'constructing sql statement upon user input', 'constructing sql statement upon user input', token)
