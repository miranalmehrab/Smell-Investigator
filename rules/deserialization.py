    ...
    try:
        ...
        if tokenType == "variable":
            ...
            if valueSrc in insecureMethods and args is not None and len(args) > 0:
                action_upon_detection(project_name, srcFile, lineno, 'insecure diserialization', 'insecure diserialization', token)

        elif tokenType == "function_call":
            if token.__contains__("name"): name = token["name"]
            if token.__contains__("args"): args = token["args"]
            if name in insecureMethods and args is not None and len(args) > 0: 
                action_upon_detection(project_name, srcFile, lineno, 'insecure diserialization', 'insecure data diserialized', token)
