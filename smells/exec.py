...
def detect(token, project_name, srcFile) :
    ...
    if tokenType == "variable":
        ...
        if token.__contains__("args"): args = token["args"]
        if valueSrc == "exec" and len(args) > 0: action_upon_detection(project_name, srcFile, lineno, 'exec_used', 'exec used')

    elif tokenType == "function_call":
        if token.__contains__("name"): name = token["name"]
        if token.__contains__("args"): args = token["args"]
        if name == "exec" and len(args) > 0: action_upon_detection(project_name, srcFile, lineno, 'exec_used', 'exec used')
        ...
        if funcReturn == "exec" and len(returnArgs) > 0: action_upon_detection(project_name, srcFile, lineno, 'exec_used', 'exec used')